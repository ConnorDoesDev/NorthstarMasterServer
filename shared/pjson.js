  ...
  for (let line of pdef.split("\n")) {
    ...
    if (type[0][0] == "$") {
      if (!currentEnumName && !currentStructName) {
        ...
        } else throw Error(`encountered unknown keyword ${type}`);
      } else if (type == ENUM_END && currentEnumName) currentEnumName = "";
      else if (type == STRUCT_END && currentStructName) currentStructName = "";
      else throw Error(`encountered unknown case with keyword ${type}`);
    }
    ...
    // normal members/struct members
      ...
      } else if (name.includes("[")) isArray = true;

      // verify type stuff
      if (
        !(
          checkType in NATIVE_TYPES ||
          checkType in ret.structs ||
          checkType in ret.enums
        )
      )
        ...
        if (isNativeArrayType) {
          ...
          newMember.nativeArraySize = parseInt(
            type.substr(type.indexOf("{") + 1)
          );
        } else if (isArray) {
          ...
          if (arraySize in ret.enums) newMember.arraySize = arraySize;
          // just use the enum name here, easier to just let people resolve this themselves
          else newMember.arraySize = parseInt(arraySize);
        }

        if (currentStructName) ret.structs[currentStructName].push(newMember);
        else ret.members.push(newMember);
      }
    }
  }
...
function GetMemberSize(member, parsedDef) {
  ...
  } else throw Error(`got unknown member type ${member.type}`);
}
    ...
    for (let member of struct) {
      ...
      for (let j = 0; j < arraySize; j++) {
        if (member.type in NATIVE_TYPES) {
          ...
          retArray.push(pdef.enums[member.type][pdata.readUInt8(i++)]);
        // enums are uint8s
        else if (member.type in pdef.structs) {
          let newStruct = {};
      ...
      base[member.name] = {
        ...
        value: member.arraySize ? retArray : retArray[0],
      };
    }
  }
  ...
  function recursiveWritePdata(struct) {
    for (let memberName in struct) {
      ...
      for (let j = 0; j < arraySize; j++) {
        ...
        if (Array.isArray(val)) val = member.value[j];
        ...
        } else if (member.type in pdef.structs) recursiveWritePdata(val);
      }
    }
  }
