...
const { GameServer, GetGameServers } = require(path.join(
  __dirname,
  "../shared/gameserver.js"
));
...
module.exports = (fastify, opts, done) => {
    ...
    async (request, reply) => {
      ...
      if (shouldRequireSessionToken) {
        ...
        let authResponse = await asyncHttp.request({
          ...
          port: 443,
          path: `/nucleus-oauth.php?qt=origin-requesttoken&type=server_token&code=${
            request.query.token
          }&forceTrial=0&proto=0&json=1&&env=production&userId=${parseInt(
            request.query.id
          )
            .toString(16)
            .toUpperCase()}`,
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      let pdata = await accounts.AsyncGetPlayerPersistenceBufferForMods(
        request.query.id,
        server.modInfo.Mods.filter((m) => !!m.pdiff).map((m) => m.pdiff)
      );
