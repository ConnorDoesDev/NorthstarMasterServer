...
const { GameServer, GetGameServers, RemoveGameServer } = require(path.join(
  __dirname,
...
module.exports = (fastify, opts, done) => {
  ...
  fastify.get("/client/servers", async (request, response) => {
    ...
    for (let i = 0; i < servers.length; i++) {
      ...
      if (
        servers[i].map == "mp_lobby" &&
        servers[i].playlist != "private_match"
      )
        continue;
    ...
    for (let server of expiredServers) RemoveGameServer(server);
